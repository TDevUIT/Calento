...
2.13. Blog & Content Tables (NEW)
2.14. AI Context & Vector Search Tables (NEW)
2.15. Relationships & Constraints
2.16. Indexes & Performance Optimization
...
2.24. Blog & CMS Endpoints (NEW)
2.25. Contact & Support Endpoints (NEW)
...
3.7. Tích hợp AI Chatbot & RAG
   - 3.7.1. Kiến trúc Function Calling
   - 3.7.2. Quản lý Ngữ cảnh & Vector Search
3.8. Hệ thống Booking Links
   - 3.8.1. Logic Tính toán Thời gian rảnh
   - 3.8.2. Quy trình Đặt lịch của Khách
3.9. Hệ thống Blog & CMS
   - 3.9.1. Quản lý bài viết

**Phần 3: Triển khai Frontend**
3.10. Tích hợp Hệ thống Xác thực
   - 3.10.1. Quản lý Trạng thái Xác thực
   - 3.10.2. Cấu hình API Client và Bảo vệ Route
3.11. Triển khai Giao diện Lịch (Calendar Views)
   - 3.11.1. Component Lịch Tùy biến
   - 3.11.2. Các Chế độ Xem và Tương tác
   - 3.11.3. Quản lý State với TanStack Query
3.12. Giao diện Chat AI Thông minh
   - 3.12.1. Hiển thị Tin nhắn và Markdown
   - 3.12.2. Action Cards và Tương tác
3.13. Trang Đặt lịch Công khai (Public Booking Page)
   - 3.13.1. Giao diện Cho Khách
   - 3.13.2. Form Đặt lịch và Tích hợp
3.14. Các trang Public (Landing, Blog, Contact)

**Phần 4: Triển khai Hạ tầng**
3.15. Kiến trúc Hạ tầng Production
   - 3.15.1. Tổng quan Kiến trúc
   - 3.15.2. Cấu hình Máy chủ (Digital Ocean Droplet)
3.16. Cấu hình Reverse Proxy và CDN
   - 3.16.1. Nginx Reverse Proxy
   - 3.16.2. Cloudflare CDN và Bảo mật
3.17. Quy trình Triển khai (Deployment Workflow)
   - Bước 1: Chuẩn bị Server
   - Bước 2: Clone Repository & Cấu hình
   - Bước 3: Build Docker Images
   - Bước 4: Khởi chạy Dịch vụ
   - Bước 5: Thiết lập Database & SSL
   - 3.17.1. Quy trình CI/CD (GitHub Actions)
3.18. Giám sát và Logging
   - 3.18.1. Hệ thống Health Check
   - 3.18.2. Chiến lược Logging
...
**Chương II:**
- Bảng 2.1: Danh sách 35+ bảng chính trong Database
...
Các tính năng chính của Calento bao gồm hệ thống xác thực an toàn với JWT và Google OAuth 2.0, quản lý sự kiện lịch đầy đủ với khả năng tạo sự kiện lặp lại theo chuẩn RRULE, đồng bộ hai chiều với Google Calendar thông qua webhooks real-time, **hệ thống Blog & CMS tích hợp**, **quản lý liên hệ (Contacts)**, AI Chatbot thông minh sử dụng Google Gemini với công nghệ **RAG & Vector Search** hiểu ngôn ngữ tự nhiên để tự động tạo sự kiện và đề xuất thời gian họp, cùng với hệ thống Booking Links cho phép chia sẻ lịch rảnh và đặt lịch công khai một cách dễ dàng.

Kiến trúc hệ thống được thiết kế theo mô hình Client-Server với Frontend sử dụng Next.js 15 kết hợp TailwindCSS, Backend xây dựng trên NestJS với kiến trúc modular, cơ sở dữ liệu PostgreSQL với **hơn 35 bảng chính** được tối ưu hóa, Redis đảm nhận vai trò cache và queue management, và toàn bộ ứng dụng được container hóa bằng Docker triển khai trên Digital Ocean với Cloudflare CDN. RESTful API với **hơn 100 endpoints** đầy đủ tài liệu Swagger đảm bảo khả năng tích hợp và mở rộng cao.
