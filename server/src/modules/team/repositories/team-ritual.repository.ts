...
export class TeamRitualRepository {
  ...
  async getNextRotationUser(ritualId: string): Promise<string | null> {
    ...
    if (
      !ritual ||
      !ritual.rotation_order ||
      ritual.rotation_order.length === 0
    ) {
      return null;
    }

    const nextIndex =
      ritual.current_rotation_index % ritual.rotation_order.length;
  ...
  async createRotationRecord(
    ritualId: string,
    userId: string,
    scheduledAt: Date,
    eventId?: string,
  ): Promise<TeamMeetingRotation> {
    const result = await this.db.query(
