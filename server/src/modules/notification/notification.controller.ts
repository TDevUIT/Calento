...
export class NotificationController {
    ...
    @ApiOperation({
        ...
        description: 'Creates notification rows for upcoming events based on their reminders settings',
    })
    ...
    async scheduleReminders(
        ...
        return this.notificationService.scheduleEventReminderNotifications(userId, days);
    }

    @Get('pending')
    @ApiOperation({
        summary: 'List pending notifications for current user (email channel)',
        description: 'Lists not-yet-sent email reminders for events owned by current user',
    })
