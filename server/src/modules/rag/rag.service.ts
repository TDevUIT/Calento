...
export class RagService {
  ...
  async retrieveConsolidatedContext(
    userId: string,
    message: string,
  ): Promise<any[]> {
    try {
      ...
      this.logger.debug(
        `Original query: "${message}" -> Expanded: "${expandedQuery}"`,
      );
      ...
      this.logger.debug(
        `Retrieved ${similarContexts.length} candidates for reranking`,
      );
  ...
  private async expandQuery(originalQuery: string): Promise<string> {
    try {
      ...
      this.logger.warn(
        `Query expansion failed, using original query. Error: ${error.message}`,
      );
  ...
  private async rerankContexts(query: string, contexts: any[]): Promise<any[]> {
    ...
    try {
      const contextList = contexts
        .map(
          (c, i) => `[${i}] ${JSON.stringify(c.context).substring(0, 300)}...`,
        )
        .join('\n');
