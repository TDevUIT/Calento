...
export const FUNCTION_DESCRIPTIONS = {
    ...
    parameters: {
      ...
      properties: {
        ...
        start_time: {
          type: STRING_TYPE,
          description:
            'Start time (ISO 8601 format with timezone offset, e.g., YYYY-MM-DDTHH:mm:ss+07:00)',
        },
        end_time: {
          type: STRING_TYPE,
          description:
            'End time (ISO 8601 format with timezone offset, e.g., YYYY-MM-DDTHH:mm:ss+07:00)',
        },
        description: {
          type: STRING_TYPE,
  ...
  ANALYZE_TEAM_AVAILABILITY: {
    ...
    description:
      'Analyze availability of multiple team members and suggest optimal meeting times with AI-powered conflict detection and match scoring',
...
export const getFunctionsByCategory = (
  category: 'calendar' | 'task' | 'analysis' | 'all',
) =>
  category === 'all'
    ? Object.values(FUNCTION_DESCRIPTIONS)
    : Object.values(FUNCTION_DESCRIPTIONS).filter(
