...
export class BlogService {
    ...
    async updateBlogPost(postId: string, dto: UpdateBlogPostDto, authorId: string): Promise<BlogPost> {
        ...
        if (existingPost.author_id !== authorId) {
            throw new BlogNotFoundException(this.messageService.get('error.unauthorized'));
        }

        const updateData: Partial<BlogPost> = {
            ...dto,
            published_at: dto.published_at ? new Date(dto.published_at) : existingPost.published_at,
            reading_time: dto.content ? this.calculateReadingTime(dto.content) : existingPost.reading_time,
        };
    ...
    async getBlogPosts(query: BlogPostQueryDto, options: Partial<PaginationOptions>): Promise<PaginatedResult<BlogPost>> {
        this.logger.log('Fetching blog posts with filters');
        return await this.blogRepository.findAll(options);
    }

    async getPublishedPosts(options: Partial<PaginationOptions>): Promise<PaginatedResult<BlogPostListItem>> {
        this.logger.log('Fetching published blog posts');
        const result = await this.blogRepository.findByStatus(BlogPostStatus.PUBLISHED, options);
        const listItems = await Promise.all(result.data.map(post => this.transformToListItem(post)));
    ...
    async getBlogPostsByCategory(categoryId: string, options: Partial<PaginationOptions>): Promise<PaginatedResult<BlogPost>> {
    ...
    async getBlogPostsByTag(tagId: string, options: Partial<PaginationOptions>): Promise<PaginatedResult<BlogPost>> {
        this.logger.log(`Fetching blog posts for tag: ${tagId}`);
        return await this.blogRepository.findByTag(tagId, options);
    }

    async getBlogPostsByAuthor(authorId: string, options: Partial<PaginationOptions>): Promise<PaginatedResult<BlogPost>> {
    ...
    async getRelatedPosts(postId: string, limit: number = 3): Promise<RelatedBlogPost[]> {
        ...
        const result = await this.blogRepository.findByCategory(currentPost.category_id, { limit: limit + 1 });
    ...
    async searchBlogPosts(searchTerm: string, options: Partial<PaginationOptions>): Promise<PaginatedResult<BlogPost>> {
